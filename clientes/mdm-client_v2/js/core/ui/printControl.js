define(function(){
	...
	return{
		...
		getLegends:function(func){
			if($.isFunction(func)){
        console.log('entra al if: '+func);
				legends = func;	
			}
			return legends();
		},
    /*********************/
    getNameLegends:function(func){
			if($.isFunction(func)){
        console.log('entra al if de la nueva funcion: '+func);
				namelegends = func;	
			}
			return namelegends();
		},

    getGrupoNameLegends:function(func){
			if($.isFunction(func)){
        console.log('entra al if deL GRUPO: '+func);
				grupoNamelegends = func;	
			}
			return grupoNamelegends();
		},
    ...
    /*******************/
		...
		getQrLinkAsDom:function(){
			...
			cadena+= '<div class="print-title2-right print-top-border">Vis&iacute;tanos en : https://sigsedum.michoacan.gob.mx/situm/ </div>';
			cadena+= '<div id="qrImage" class="qrImage"><img src="img/print/QR_SIG_SEDUM.png" /></div>';
			return cadena;
		},
		getLegendsAsDom:function(){
			var obj = this;
			var legends = obj.getLegends();
      var nleyenda = obj.getNameLegends();
      var gnleyenda = obj.getGrupoNameLegends();
			var cadena = '';
			cadena+= '<div class="print-title2 print-top-border">Leyendas de informaci&oacute;n</div>';
      cadena+= '<div class="contenedor-legend">';
			for(var x in legends){
        //cadena+= '<div class="title-legends">'+gnleyenda[x]+'</div>';
        cadena+= '<div class="title-legends print-info-block width-50">'+gnleyenda[x]+': '+nleyenda[x]+'<br><img src="'+legends[x]+'"/></div>';
				//cadena+= '<div class="print-info-block width-50"><img src="'+legends[x]+'"/></div>';	
			}
