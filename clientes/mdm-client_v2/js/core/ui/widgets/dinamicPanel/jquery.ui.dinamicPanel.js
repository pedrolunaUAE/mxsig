$.widget("custom.dinamicPanel", {
  ...
  panels: [
    // { id: 'layerPanel', type_layer: 'layer', height: 180, label: 'Capas', desc: 'Filtrado de capas', icon: 'fa-solid fa-layer-group'},
    { id: 'leyendPanel', type: 'leyend', label: 'Leyenda', desc: 'Significado de la simbolog&iacute;a del mapa', icon: 'fa-solid fa-list' },
    { id: 'bufferPanel', type: 'buffer', label: 'An&aacute;lisis', desc: 'Análisis espacial', icon: 'fa-solid fa-globe-americas' },
    { id: 'geoPanel', type: 'geo', label: 'Georef', desc: 'Digitalizar en el mapa', icon: 'fa-solid fa-map-location-dot' },
    { id: 'measurePanel', type: 'measure', label: 'Medir', icon: 'fa-solid fa-ruler', desc: 'Realiza mediciones en el mapa', data: {}, list: [] },
    //{ id: 'routePanel', type: 'route',  label: 'Ruteo', desc: 'Crear rutas por las v&iacute;as principales del pa&iacute;s', icon: 'fa-solid fa-route' },
     { id: 'searchPanel', type: 'search', label: 'Resultado', desc: 'Resultados de b&uacute;squeda', icon: 'fa-solid fa-magnifying-glass' },
  ...
  /***************************/
  getNameLegendList: function () {
    var obj = this;
    obj.refreshLegend();
    return obj.nombresLegends;
  },

  getGrupoNameLegendList: function () {
    var obj = this;
    obj.refreshLegend();
    return obj.grupoNameLegends;
  },
...
/*+++++++++*/
    var listCapaName = obj.options.layers;
/*+++++++++*/    
...
/************************* */
    ...
    for (var x in list) {
      ...
      listLayer.push(list[x].label);
      mylistaN[x]=list[x].idGroup
      /*+++++++++++++++*
      listaName[list[x].idGroup] = {layer: list[x] };
      /*+++++++++++*/
    };
/****************
    for( var i in listaName){
      var nomC = '';
      nomC = listCapaName[i].label;
      mylistaN.push(listCapaName[i].label);
    }
/***************/
    var filerList = [];
    ...
    for (var x in list2) {
      ...
      if (x.substr(0, 4) == 'img_') {
        ...
        if (customLegend) {
          ...
          nl=listLayer[cont];
          gnl=listCapaName[mylistaN[cont]].label;
          //cadena += '<div class = "nomC">'+listCapaName[mylistaN[cont]].label+'<br>'+listLayer[cont]+'</div>';
          cadena += '<div class = "nomC">';
          cadena += '<div class="tit">' +listCapaName[mylistaN[cont]].label+ '</div>'; 
          cadena += '<div class="sub">'+listLayer[cont]+'</div>';
          cadena += '</div>';
          //cadena += '<div class = "nomC">'+listLayer[cont]+'</div>';
          //cadena += '<div class = "imagen">';
          cadena += '<img src="' + image + '" />';
          //cadena += '</div>';
      ...
      obj.grupoNameLegends.push(gnl);
      obj.nombresLegends.push(nl);
    ...
    cadena = (cadena != '') ? '<div class = "btnWindow"><span id="' + obj.id + '_legendWindow" class="dinamicPanel-legendWindow dinamicPanel-sprite dinamicPanel-window-short" /></div><div class="dinamicPanel-leyend-container">' + cadena + '</div>' : '';
  ...
  createPanel: function (type, data) {
    ...
    switch (type) {
      ...
      case 'geo':
        ...
        cadena += '      <button id="' + obj.id + '_geo_btnEndAction" style="display:' + ((isNew) ? 'none' : '') + '">Cancelar</button>';
        cadena += '      <button id="' + obj.id + '_geo_btnFinish" style="display:' + ((isNew) ? 'none' : '') + '">Terminar</button>';
        ...
        cadena += '          <button id="' + obj.id + '_geo_btnCreateBuffers" class="geoConvert" style="display:' + ((isNew) ? '' : 'none') + '">Generar &aacute;reas de influencia</button>';
      ...
      case 'route':
        ...
        break;
      case 'measure':
        $('#layersDisplay').css('display', 'none'); //Cierra ventana capas
        var cadena = '';
        data = (data === undefined) ?
          { data: { name: '', type: '', measure: 0, unit: 'metric' }, id: '' }
          : data;
        ...
        cadena += '   <div class="measureTitle">Medir</div> ';
        cadena += '   <div class="measureOption">';
        cadena += '      <div style="display:' + ((isNew) ? '' : 'none') + '" id="' + obj.id + '_measure_typeSet" class="type">';
        cadena += '           <input type="radio" ' + tl + ' id="' + obj.id + '_measure_line" class="measureType" value="measureLine" idref="line" name="measure_type"/><label for="' + obj.id + '_measure_line">Distancia</label>';
        cadena += '           <input type="radio" ' + tp + ' id="' + obj.id + '_measure_poly" class="measureType" value="measurePolygon" idref="poly" name="measure_type"/><label for="' + obj.id + '_measure_poly">&Aacute;rea</label>';
        cadena += '      </div>';
        cadena += '      <button id="' + obj.id + '_measure_btnEndAction" style="display:' + ((isNew) ? 'none' : '') + '">Cancelar</button>';
        cadena += '      <button id="' + obj.id + '_measure_btnFinish" style="display:' + ((isNew) ? 'none' : '') + '">Terminar</button>';
        cadena += '   </div>';
  ...
  createPolyList: function (list) {
    ...
    var getItemTools = function (item, pos) {
      var labels = {
        selection: 'Procesar selecci&oacute;n',
  ...
  createGeoList: function (list) {
    ...
    var getItemTools = function (item, pos) {
      var labels = {
        geobuffer: 'Generar &aacute;rea de influencia',
        edit: 'Editar el elemento',
    ...
    if (typeof (list) != 'undefined') {
      ...
      for (var x in list) {
        ...
        cadena += '  <div class="iconIndicator"><span class="dinamicPanel-geoIcon ' + clases + '"></span></div>';
        if (hasHTML) {
        ...
        cadena += '  </div>';
        cadena += '  <div class="crossBtn"><span pos="' + x + '" idref="' + item.id + '" class="geo-rem-button dinamicPanel-sprite dinamicPanel-close-short"></span></div>';
  ...
  createPointList: function (list) {
    ...
    var getItemTools = function (item, pos) {
      var labels = {
        buffer: 'Asignar una &Aacute;rea de influencia',
        edit: 'Editar el elemento'
      };
      var tools = item.tools;
      var cadena = '';
  ...
  createTabs: function () {
    var obj = this;
    var cadena = '';
    var width = 58; //Math.floor(100/obj.panels.length);
    for (var x in obj.panels) {
      var item = obj.panels[x];
      //********verificar relacion entre searchPanel******
      if(item.id != "searchPanel")
      cadena += '<div style="width:' + width + 'px" id="' + obj.id + '_' + item.id + '_tab" idref="' + item.id + '" alt="' + item.desc + '"  title="' + item.desc + '" class="dinamicPanel-tab"><div class="dinamicPanel-tabIconContainer"><center><span title="' + item.desc + '" class="' + item.icon + '"></span></center></div><span class="dinamicPanel-tabDescription">' + item.label + '</span><span class="dinamicPanel-selector dinamicPanel-hidde"></span></div>';
    };
    return cadena;
  },
  ...
  showDetail: function (pos, type, hidde, byText) {
    ...
    if (type == 'content') { //este boque solo se usa para sustituciones completas
      ...
      if (!(pos === undefined)) {
        ...
        var extent = '';
        var coordenada = '';
        var img = 'https://maps.googleapis.com/maps/api/streetview?size=100x80&location=46.414382,10.013988&heading=151.78&pitch=-0.76&sensor=false';
        var cadena = '<div class="dinamicPanel-detailContent">';
            ...
            if (type == 'identify') {
              ...
              if (typeof (item.value) == 'string' && item.value.indexOf('|') >= 0) { //es un campo especial
                ...
                if (values.length > 2) {
                  ...
                  item.value = (item.value.search('https') > -1) ? '<a href="' + item.value.split('|')[1] + '" target="_blank">' + item.value.split('|')[0] + '</a>' : item.value;
 ...
 initSearchCross : function (text, func) {
  ...
  adjustResultHeight: function (height) {
    ...
    // var element = document.getElementById("#mdm6DinamicPanel_results");
    // element.classList.add("routing-point");

    rheight = (rheight - posTop) - 90;

    height = (height === undefined) ? $(window).height() : height;
    //if (height < 750){
    // $('#mdm6DinamicPanel_results').classList.add("results-config");
  ...
  _refresh: function () {
    ...
    cadena += '      <span id="' + obj.id + '_btnClearSearch" class="dinamicPanel-clearSearch fa-solid fa-delete-left"></span>';
    ...
    cadena += '         <div id="' + obj.id + '_layerManager_btnLayerConf" idref="layerPanel" alt="Filtrado de capas"  title="Filtrado de capas" class="dinamicPanel-tab" style="width: 58px">';
    cadena += '           <div class="dinamicPanel-tabIconContainer">';
    cadena += '             <center>';
    cadena += '               <span title="Filtrado de capas" class="fa-solid fa-layer-group"></span>';
    cadena += '             </center>';
    cadena += '           </div>';
    cadena += '           <span class="dinamicPanel-tabDescription"> Capas </span>';
    cadena += '           <span class="dinamicPanel-selector dinamicPanel-hidde"></span>';
    cadena += '         </div>';
    cadena += obj.createTabs();
    cadena += '         <div  idref="contactPanel" alt="Contacto" class="dinamicPanel-tab dropdown" style="width: 58px">';
    cadena += '           <div class="dinamicPanel-tabIconContainer">'
    cadena += '             <center>';
    cadena += '               <span class="fa-solid fas fa-angle-down"></span>';
    cadena += '             <center>';
    cadena += '           </div>';
    cadena += '           <span class="dinamicPanel-tabDescription"> EI2025 </span>';
    cadena += '           <span class="dinamicPanel-selector dinamicPanel-hidde"></span>';
    cadena += '           <div class="dropdown-content">';
   // cadena += '             <a href="https://forms.gle/g95EPdHYq9ZSkNCN9" target="_blank"><i class="fa-regular fa-clipboard"></i> Evaluaci&oacuten del sitio</a>',
    cadena += '             <a href="docs/musua_mdmlinea.pdf" target="_blank"><i class="fas fa-book"></i> Manual de Usuario</a>',
    cadena += '             <a href="http://geogat046068d.inegi.gob.mx:8888/rep2024/ageb/carga.php?entidad=18&filtro=filtro1" target="_blank"><i class="far fa-file"></i> Reporte de Avance</a>';
    cadena += '             <a href="docs/Reporte_EI2025.pdf" target="_blank"><i class="far fa-file-alt"></i> Reporte de Campo</a>',
    cadena += '             <a href="http://10.73.1.106/descargas/" target="_blank"><i class="fas fa-download"></i> Descarga MG PDF </a>';
 //   cadena += '             <a href="https://twitter.com/Sedum_mich" target="_blank"><i class="fa-brands fa-twitter"></i> Twitter </a>';
    cadena += '             <a href="https://maps.app.goo.gl/cv8C1cDRuv8PYdkLA" target="_blank"><i class="fas fa-map-marker-alt"></i> INEGI CE-Nayarit </a>';
    cadena += '           </div>'; 
    cadena += '         </div>';
    cadena += '      </div>';
    ...
    //area de herramientas
    cadena += '<div id="' + obj.id + '_results" style="display:none" visible="false" class="">';
...
//****************************************************************************************** */
    ...
    $('#mdm6DinamicPanel_leyendPanel_tab').click(function () {
      MDM6('hideMarkers', 'identify');
    });
